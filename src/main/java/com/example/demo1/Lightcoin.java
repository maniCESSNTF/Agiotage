...
public class Lightcoin {
    ...
    private void updateTime() {
        ...
        txtNowPrice.setText(String.valueOf(Double.valueOf(df.format(toman))));
        txtPercentage.setText(String.valueOf(Double.valueOf(df.format(((toman - toman24) / toman) * 100))));
//        txtVolume-----------------------------------------------------حجم معاملات
    ...
    void Pbtnset(ActionEvent event) throws IOException, SQLException {
        ...
        String da =Now.formatDate(milliNow-24*60*60*1000L) ;

        try {
            Connection conn = DriverManager.getConnection(url, user, password);
            String initialEmail = da;
            //System.out.printf("lllllllll"+da);
...
//        txtVolume-----------------------------------------------------حجم معاملات
        ...
        try (Connection connection1 = DriverManager.getConnection("jdbc:mysql://localhost:3306/agiotage2", "root", "")) {
            ...
            try (PreparedStatement statement1 = connection1.prepareStatement(sql1)) {
                statement1.setString(1, "lightcoin");
